...
void UserMgr::throwForDeleteUserInServiceGroup(const std::string &userName) {
  ...
  if (std::find(groupLists.begin(), groupLists.end(), "service") !=
      groupLists.end()) {
    lg2::error(
        "Not allowed to delete user belongs to service group '{USERNAME}'",
        "USERNAME", userName);
    elog<NotAllowed>(
...
void UserMgr::throwForMaxGrpUserCount(
  ...
  if (std::find(groupNames.begin(), groupNames.end(), "ipmi") !=
    ...
    if (getRedfishHostInterfaceUsersCount() >= redfishHostInterfaceUsers) {
      lg2::error("Redfish Host Interface user limit reached");
      elog<NoResource>(xyz::openbmc_project::User::Common::NoResource::REASON(
...
void UserMgr::throwForInvalidGroups(
    const std::vector<std::string> &groupNames) {
...
void UserMgr::createUser(std::string userName,
  ...
  try {
    ...
    lg2::error("Unable to create new user '{USERNAME}'", "USERNAME", userName);
...
void UserMgr::renameUser(std::string userName, std::string newUserName) {
  ...
  sendEvent(MESSAGE_TYPE::PROPERTY_VALUE_MODIFIED, Entry::Level::Informational,
            messageArgs, dbusObjectPath);
  return;
}
...
void UserMgr::updateGroupsAndPriv(const std::string &userName,
  ...
  try {
    ...
    lg2::error("Unable to modify user privilege / groups for user '{USERNAME}'",
               "USERNAME", userName);
    elog<InternalFailure>();
  }
...
uint8_t UserMgr::minPasswordLength(uint8_t value) {
  ...
  if (setPamModuleConfValue(pwQualityConfigFile, minPasswdLenProp,
    ...
    lg2::error("Unable to set minPasswordLength to {VALUE}", "VALUE", value);
    elog<InternalFailure>();
  }
...
uint16_t UserMgr::maxLoginAttemptBeforeLockout(uint16_t value) {
  ...
  if (setPamModuleConfValue(faillockConfigFile, maxFailedAttempt,
                            std::to_string(value)) != success) {
    lg2::error("Unable to set maxLoginAttemptBeforeLockout to {VALUE}", "VALUE",
               value);
    elog<InternalFailure>();
  }
...
uint32_t UserMgr::accountUnlockTimeout(uint32_t value) {
  ...
  if (setPamModuleConfValue(faillockConfigFile, unlockTimeout,
                            std::to_string(value)) != success) {
    lg2::error("Unable to set accountUnlockTimeout to {VALUE}", "VALUE", value);
    elog<InternalFailure>();
  }
  if (setPamModuleConfValue(faillockConfigFile, rootUnlockTimeoutProp,
                            std::to_string(value)) != success) {
    lg2::error("Unable to set rootUnlockTimeout to {VALUE}", "VALUE", value);
    elog<InternalFailure>();
  }
...
int UserMgr::getPamModuleConfValue(const std::string &confFile,
  ...
  if (!fileToRead.is_open()) {
    lg2::error("Failed to open pam configuration file {FILENAME}", "FILENAME",
               confFile);
    return failure;
  }
  std::string line;
  ...
  while (getline(fileToRead, line)) {
    ...
    if ((startPos = line.find(argSearch)) != std::string::npos) {
      ...
      fileToWrite << line.substr(0, startPos) << argValue
                  << line.substr(endPos, line.size() - endPos) << std::endl;
      found = true;
      continue;
    }
    fileToWrite << line << std::endl;
...
UserSSHLists UserMgr::getUserAndSshGrpList() {
  ...
  while (true) {
    auto r =
        fgetpwent_r((passwd)(), &pw, buffer.data(), buffer.max_size(), &pwp);
    if ((r != 0) || (pwp == NULL)) {
    ...
    // and special UID 0.
    if ((pwp->pw_uid == 0) || ((pwp->pw_uid >= 1000) && (pwp->pw_uid < 65534)))
#else
    // Add all users whose UID >=1000 and < 65534
...
UserMgr::getUsersInGroup(const std::string &groupName) {
...
DbusUserObj UserMgr::getPrivilegeMapperObject(void) {
  ...
  try {
    ...
    lg2::error("Failed to excute GetManagedObjects at {PATH}: {ERR}", "PATH",
               ldapMgrObjBasePath, "ERR", e);
    throw;
  }
  return objects;
}
...
gid_t UserMgr::getPrimaryGroup(const std::string &userName) const {
  ...
  auto status =
      getpwnam_r(userName.c_str(), &pwd, buffer.data(), buffer.size(), &pwdPtr);
...
bool UserMgr::isGroupMember(const std::string &userName, gid_t primaryGid,
  ...
  while (status == ERANGE && buflen < maxBufferLength) {
    ...
    lg2::debug("Increase buffer for getgrnam_r() to {SIZE}", "SIZE", buflen);

    status = getgrnam_r(groupName.c_str(), &grp, buffer.data(), buffer.size(),
                        &grpPtr);
  }
...
UserInfoMap UserMgr::getUserInfo(std::string userName) {
  ...
  if (isUserExist(userName)) {
    ...
    userInfo.emplace("UserPasswordExpired", user.get()->userPasswordExpired());
    ...
    try {
      for (const auto &[path, interfaces] : objects) {
        ...
        if (it != interfaces.end()) {
          ...
          if (propIt != it->second.end() && std::get<bool>(propIt->second)) {
      ...
      for (const auto &[path, interfaces] : objects) {
        ...
        auto it =
            interfaces.find("xyz.openbmc_project.User.PrivilegeMapperEntry");
        if (it != interfaces.end()) {
...
void UserMgr::initializeAccountPolicy() {
  ...
  unsigned long tmp = 0;
  if (getPamModuleConfValue(pwQualityConfigFile, minPasswdLenProp, valueStr) !=
      success) {
  ...
  valueStr.clear();
  if (getPamModuleConfValue(pwHistoryConfigFile, remOldPasswdCount, valueStr) !=
      success) {
    ...
    try {
      ...
      lg2::error("Exception for RememberOldPasswordTimes: {ERR}", "ERR", e);
      throw;
    }
  ...
  if (getPamModuleConfValue(faillockConfigFile, maxFailedAttempt, valueStr) !=
    ...
    try {
      ...
      lg2::error("Exception for MaxLoginAttemptBeforLockout: {ERR}", "ERR", e);
      throw;
    }
  ...
  if (!userNameList.empty()) {
    ...
    for (auto &user : userNameList) {
      ...
      for (const auto &grp : groupLists) {
        ...
        if (std::find(tempGrp.begin(), tempGrp.end(), user) != tempGrp.end()) {
...
UserMgr::UserMgr(sdbusplus::bus_t &bus, const char *path)
  ...
  if (minPasswdLength <
    ...
    if (setPamModuleConfValue(
      ...
      lg2::error("Unable to set minLowerCase to {VALUE}", "VALUE", MIN_DIGITS);
    }
