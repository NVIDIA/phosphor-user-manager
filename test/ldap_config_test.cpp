...
class TestLDAPConfig : public testing::Test
{
  public:
    ...
    void SetUp() override
    {
        ...
        tlsCACertFile = tlsCacertFilePath.filename();
        fs::path tlsCertFilePath{TLS_CERT_FILE};
        tlsCertFile = tlsCertFilePath.filename();

        fs::path confFilePath{LDAP_CONFIG_FILE};
        ldapConfFile = confFilePath.filename();
        std::fstream fs;
...
TEST_F(TestLDAPConfig, testRestores)
{
    ...
    MockConfigMgr* managerPtr = new MockConfigMgr(
        bus, sdbusplus::object_path(LDAP_CONFIG_ROOT), configFilePath,
        dbusPersistentFilePath, tlsCACertFilePath, tlsCertFilePath);
...
TEST_F(TestLDAPConfig, filePermission)
{
    ...
    auto permission = fs::perms::owner_read | fs::perms::owner_write |
                      fs::perms::group_read;
    auto persistFilepath = dir.string();
...
TEST_F(TestLDAPConfig, testNewlineRejection)
{
    ...
    MockConfigMgr* managerPtr = new MockConfigMgr(
        bus, sdbusplus::object_path(LDAP_CONFIG_ROOT), configFilePath.c_str(),
        dbusPersistentFilePath.c_str(), tlsCACertFilePath.c_str(),
        tlsCertFilePath.c_str());
